...
# 1. Agendamento para INICIAR instâncias de computação diariamente às 08:30 BRT
resource "oci_resource_scheduler_schedule" "start_instances_schedule" {
  ...
  description        = "Agendamento para iniciar todas as instâncias de computação diariamente"
...
# 2. Agendamento para PARAR instâncias de computação diariamente às 18:30 BRT
resource "oci_resource_scheduler_schedule" "stop_instances_schedule" {
  count              = var.enable_resource_scheduler ? 1 : 0
  action             = "STOP_RESOURCE"
  compartment_id     = var.compartment_ocid
  display_name       = "sched-stop-vms-1830-brt"
  description        = "Agendamento para parar todas as instâncias de computação diariamente"
  recurrence_type    = "CRON"
