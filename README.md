...
![Licença](https://img.shields.io/badge/Licenca-MIT-green.svg)

Este repositório contém Infraestrutura como Código (IaC) escrita em **Terraform**, organizada sob uma arquitetura de **2 Stacks Independentes** para a **Oracle Cloud Infrastructure (OCI)**.
...
oci/
├── .github/workflows/
│   └── provisioning-iac-oci.yml # Pipeline de CI/CD GitOps (GitHub Actions)
│
├── iac/                        # 🛠️ Infraestrutura como Código (Terraform)
│   ├── remote-backend-stack/  # Stack 1: Provisiona o Bucket de Object Storage para o tfstate remoto
...
## 🛠️ Arquitetura das Stacks (`iac/`)

O código de automação está separado dentro da pasta **`iac/`** em 2 stacks independentes para garantir idempotência e evitar dependências circulares de estado:

1. **`iac/remote-backend-stack/`**:
   * **Objetivo**: Cria um **Bucket de Object Storage na OCI** (Always Free - 20GB) para armazenar o arquivo `terraform.tfstate` remotamente com versionamento habilitado.
   * **Ordem de Execução**: Deve ser executado **primeiro**.

2. **`iac/main-stack/`**:
   * **Objetivo**: Provisiona um **Cluster Kubernetes Nativo de 3 Nós Privados** + **1 Bastion Host** executando em **Oracle Linux 9 (OL9)** (nível 100% Always Free: 4 OCPUs e 24GB RAM ARM Flex + 1 OCPU AMD Micro).
   * **Ordem de Execução**: Deve ser executado **após** a criação do backend remoto.
...
Após o `terraform apply` criar o Bastion e as 3 VMs na OCI, configure e instale o Kubernetes nativo e o conjunto de ferramentas SRE navegando até o projeto Ansible:

```bash
# 1. Navegue até o diretório do projeto Ansible
cd ../ansible/deploy

# 2. Copie o arquivo de inventário de exemplo da OCI
...
Para habilitar a automação no GitHub Actions, registre os seguintes **Repository Secrets** em **Settings > Secrets and variables > Actions**:
...
| `OCI_REGION` | (Opcional) Região da OCI (Padrão: `us-ashburn-1`) | `us-ashburn-1` ou `sa-saopaulo-1` |
| `SSH_PUBLIC_KEY` | Conteúdo da sua chave pública SSH (`cat ~/.ssh/id_rsa.pub`) | `ssh-rsa AAAAB3NzaC1yc2E...` |

> ⚠️ **Nota Importante sobre o `SSH_PUBLIC_KEY`**: O secret `SSH_PUBLIC_KEY` deve conter o texto da sua chave pública (ex.: `cat ~/.ssh/id_rsa.pub`) em **formato OpenSSH** (linha única iniciando com `ssh-rsa` ou `ssh-ed25519`). Não utilize o formato PEM (`-----BEGIN PUBLIC KEY-----`), pois a OCI irá rejeitar com erro HTTP 400.

#### Fluxo da Pipeline:
* **Pull Request**: Dispara automaticamente o `terraform plan` em alterações dentro de `iac/**`.
* **Push para a `main` / Disparo Manual**: Permite executar `terraform apply` ou `terraform destroy` em **Actions > Terraform OCI (Plan, Apply, Destroy) > Run workflow**.

---

## 🛡️ Segurança e Permissões IAM (Princípio do Menor Privilégio)
